...
SELECT
    p.client_id,
    COALESCE(ps_exp.projected_savings_expenses,0) + COALESCE(ps_table.total_savings,0) AS total_savings,
...
-- Debt payments per client

WITH params AS (
    -- Set the client_id once
    SELECT 2::INT AS client_id,
           DATE_TRUNC('year', CURRENT_DATE) AS current_year_start
),
projected_debt_cte AS (
    -- Projected / Planned Debt (from core.expenses)
    SELECT
        p.client_id,
        COALESCE(SUM(e.annual_amount), 0) AS projected_debt
    FROM
        params p
    JOIN
        core.expenses e ON e.client_id = p.client_id
    WHERE
        (
            LOWER(e.type) LIKE '%loan%' OR LOWER(e.type) LIKE '%debt%' OR LOWER(e.type) LIKE '%mortgage%' OR LOWER(e.type) LIKE '%credit%' OR
            LOWER(e.sub_type) LIKE '%loan%' OR LOWER(e.sub_type) LIKE '%debt%' OR LOWER(e.sub_type) LIKE '%mortgage%' OR LOWER(e.sub_type) LIKE '%credit%'
        )
        -- Start date is before the end of the current year, and projection is active this year or later
        AND e.start_actual_date <= p.current_year_start + INTERVAL '1 year' - INTERVAL '1 day'
        AND (e.end_projection_date >= p.current_year_start OR e.end_projection_date IS NULL)
    GROUP BY
        p.client_id
...
actual_debt_cte AS (
    -- Actual Debt Paid (from core.flows)
    SELECT
        p.client_id,
        COALESCE(SUM(f.amount), 0) AS actual_debt
    FROM
        params p
    JOIN
        core.flows f ON f.client_id = p.client_id
    WHERE
        (
            LOWER(f.fact_type_name) LIKE '%loan%' OR LOWER(f.fact_type_name) LIKE '%mortgage%' OR LOWER(f.fact_type_name) LIKE '%credit%'
        )
        -- Flow occurred during the current calendar year
        AND f.amount_as_of >= p.current_year_start
        AND f.amount_as_of < p.current_year_start + INTERVAL '1 year'
    GROUP BY
        p.client_id
...
liability_cte AS (
    -- Outstanding Debt and Annual Payment (from core.liability_note_accounts)
    SELECT
        p.client_id,
        COALESCE(SUM(l.total_value), 0) AS outstanding_debt,
        COALESCE(SUM(
            CASE
                ...
                ELSE
                    l.total_value / NULLIF(l.loan_term_in_years, 0) -- Use NULLIF to prevent division by zero
            END
        ), 0) AS annual_debt_payment
    FROM
        params p
    JOIN
        core.liability_note_accounts l ON l.client_id = p.client_id
    GROUP BY
        p.client_id
)
...
SELECT
    p.client_id,
    COALESCE(e.projected_debt, 0) AS projected_debt,
    COALESCE(f.actual_debt, 0) AS actual_debt,
    COALESCE(l.outstanding_debt, 0) AS outstanding_debt,
    COALESCE(l.annual_debt_payment, 0) AS annual_debt_payment
FROM
    params p
LEFT JOIN projected_debt_cte e ON p.client_id = e.client_id
LEFT JOIN actual_debt_cte f ON p.client_id = f.client_id
LEFT JOIN liability_cte l ON p.client_id = l.client_id;
...
planned_living_cte AS (
    -- Projected / Planned Living Expenses from expenses
    SELECT
        p.client_id,
        COALESCE(SUM(e.annual_amount), 0) AS planned_living_expenses
    FROM
        params p
    LEFT JOIN core.expenses e ON e.client_id = p.client_id
    WHERE
...
SELECT
    p.client_id,
    COALESCE(pl.planned_living_expenses, 0) AS planned_living_expenses,
    COALESCE(al.actual_living_expenses, 0) AS actual_living_expenses
FROM
    params p
LEFT JOIN planned_living_cte pl ON p.client_id = pl.client_id
LEFT JOIN actual_living_cte al ON p.client_id = al.client_id;
